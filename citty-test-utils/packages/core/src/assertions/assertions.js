...
export function wrapExpectation(result) {
  ...
  return {
    ...
    expectJson(fn) {
      if (!result.stdout) {
        throw new Error('Expected valid JSON output, but stdout is empty')
      }
      const json = destr(result.stdout)
      if (typeof json !== 'object' || json === null || (typeof json === 'string' && json === result.stdout && !result.stdout.trim().startsWith('{') && !result.stdout.trim().startsWith('['))) {
